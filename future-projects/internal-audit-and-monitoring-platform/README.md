...
**MVP**: Event-driven graph: metrics → anomaly → investigate → policy check → remediate or escalate; plus on-demand QueryAgent.

| Milestone | Deliverable |
|-----------|-------------|
| 1. Anomaly stream | Synthetic metrics + baseline/seasonality |
| 2. Investigation | Deep-dive node with evidence pack |
| 3. Policy gate | Block remediations that violate rules |
| 4. Dual mode | Continuous path + on-demand Q&A |

**Done when**: Planted anomalies are detected and either fixed under policy or escalated; no unapproved remediations.
**Estimate**: 3.5 weeks · **Target finish**: Apr 23, 2027
