...
**MVP (capstone)**: Client inquiry → intake → estimate → architecture → implement → deploy → monitor, with finance + feedback agents and escalation.

| Milestone | Deliverable |
|-----------|-------------|
| 1. Phase graph | Full node set with approval/renegotiate edges |
| 2. Reuse upstream | Pull harness/loop/context patterns from prior projects |
| 3. Simulation | One fictional client engagement end-to-end |
| 4. Ops metrics | Cost tracking, phase latency, escalation rate |

**Done when**: A simulated engagement completes all phases with human gates at estimate/architecture; blockers escalate with context; finance sees cost trail.
**Estimate**: 4 weeks · **Target finish**: Jun 18, 2027
