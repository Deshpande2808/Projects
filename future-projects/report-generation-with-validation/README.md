...
**MVP**: Multimodal/raw field inputs → schema-valid JSON report → repair loop → human sign-off queue.

| Milestone | Deliverable |
|-----------|-------------|
| 1. Schema | Strict report JSON Schema + validators |
| 2. Extraction | Text (+ optional image/voice) → fields |
| 3. Verify/repair | Completeness + sanity checks; regenerate bad fields |
| 4. Escalation | After 3 failures, mark fields for human |

**Done when**: Invalid schema never leaves the loop; planted bad extractions are corrected or escalated.
**Estimate**: 2 weeks · **Target finish**: Jan 30, 2027
